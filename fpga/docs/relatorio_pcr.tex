...
\subsection{Sistema em Chip}

O acelerador foi empacotado como IP e integrado ao ARM Cortex-A9 do Zynq num
\emph{block design} (Fig.~\ref{fig:bd}). O ARM mant\'em a varredura na DDR; o
AXI-DMA a l\^e pela porta \texttt{S\_AXI\_HP0} e a entrega ao acelerador como
\emph{stream}; as \num{19} folgas retornam por AXI4-Lite e a
\texttt{IRQ\_F2P} sinaliza a conclus\~ao.

% O write_bd_layout do Vivado gera a pagina em paisagem com /Rotate 270, de modo
% que a figura entra deitada. O rotatebox{-90} a coloca de pe; o resizebox a
% ajusta a largura das duas colunas. Sem o PDF, desenha-se o esquema equivalente.
\begin{figure*}[!t]
\centering
\IfFileExists{figs/block_design.pdf}{%
  % 73%% da pagina do Vivado e branco: o trim recorta ate o conteudo.
  \resizebox{\textwidth}{!}{\rotatebox{-90}{%
    \includegraphics[trim=211 30 209 36, clip]{figs/block_design.pdf}}}%
}{%
  \begin{tikzpicture}[font=\small, node distance=10mm,
    blk/.style={draw, rounded corners=2pt, minimum height=12mm,
                minimum width=26mm, align=center, inner sep=4pt},
    arr/.style={-Latex, thick}]
    \node[blk, fill=green!10] (ps) {\textbf{Zynq PS}\\ARM Cortex-A9\\\scriptsize DDR};
    \node[blk, right=16mm of ps, fill=blue!10] (dma) {\textbf{AXI-DMA}};
    \node[blk, right=16mm of dma, fill=orange!14] (acc)
         {\textbf{lidar\_accel}\\\scriptsize CORDIC $+$ 19 pistas};
    \draw[arr] (ps) -- node[above, font=\tiny] {AXI4-Lite} (dma);
    \draw[arr] (dma) -- node[above, font=\tiny] {AXI4-Stream} (acc);
    \draw[arr] (ps.south) -- ++(0,-6mm) -|
      node[pos=0.25, below, font=\tiny] {AXI4-Lite: calibra\c{c}\~ao $+$ 19 folgas} (acc.south);
    \draw[arr, red] (acc.north) -- ++(0,6mm) -|
      node[pos=0.25, above, font=\tiny] {IRQ} (ps.north);
  \end{tikzpicture}%
}
\caption{\emph{Block design} no Vivado. O ARM (\texttt{zynq}) alcan\c{c}a o
acelerador e o DMA por AXI4-Lite atrav\'es do \texttt{zynq\_axi\_periph}. O
\texttt{dma} l\^e a varredura da DDR (\texttt{M\_AXI\_MM2S} $\to$
\texttt{axi\_mem\_intercon} $\to$ \texttt{S\_AXI\_HP0}) e a entrega ao
\texttt{accel} por AXI4-Stream (\texttt{M\_AXIS\_MM2S} $\to$ \texttt{s\_axis}).
A conclus\~ao retorna por \texttt{irq} $\to$ \texttt{IRQ\_F2P}, dispensando
espera ocupada no processador.}
\label{fig:bd}
\end{figure*}
...
\begin{semilogxaxis}[width=\columnwidth, height=4cm,
  ...
  ytick={HW150,HW100,NumPy,escalar}, y dir=reverse,
