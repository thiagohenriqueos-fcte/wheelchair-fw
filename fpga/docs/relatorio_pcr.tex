...
%
%  ==> CAMPOS A PREENCHER APOS RODAR O VIVADO: procure por "\vivado{" no texto.
%      Os valores estao em fpga/vivado/reports/.
% =============================================================================
...
% Marca os numeros que so o Vivado fornece. Depois de rodar a sintese, troque
% \vivado{X} pelo valor -- ou redefina a macro para imprimir sem destaque.
\newcommand{\vivado}[1]{\textcolor{red}{\textbf{#1}}}
...
Vale interpretar o ganho, e n\~ao apenas exibi-lo. O LiDAR entrega uma varredura a
cada \SI{100}{\milli\second}: mesmo o software vetorizado consome apenas
\SI{0.5}{\percent} desse or\c{c}amento, de modo que \textbf{o acelerador n\~ao
``salva'' o sistema} --- ele n\~ao estava perdendo prazos. O valor real \'e outro,
...
\begin{table}[!t]
\caption{S\'intese e implementa\c{c}\~ao (Vivado, \texttt{xc7z020})}
\label{tab:util}
\centering\footnotesize
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Recurso} & \textbf{Usado} & \textbf{Dispon\'ivel} & \textbf{\%} \\
\midrule
LUT        & \vivado{---} & \num{53200} & \vivado{---} \\
Flip-flop  & \vivado{---} & \num{106400} & \vivado{---} \\
DSP48      & \vivado{---} & \num{220}   & \vivado{---} \\
BRAM       & \vivado{---} & \num{140}   & \vivado{---} \\
\midrule
WNS (folga) & \multicolumn{3}{l}{\vivado{--- ns}} \\
$F_{\max}$  & \multicolumn{3}{l}{\vivado{--- MHz}} \\
Pot\^encia total & \multicolumn{3}{l}{\vivado{--- W}} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Previs\~ao anal\'itica} (a ser confrontada com a
Tab.~\ref{tab:util}): \num{78} DSP48 (\SI{35}{\percent}) --- \num{76} das pistas
mais \num{2} da proje\c{c}\~ao; nenhum BRAM, pois n\~ao h\'a mem\'oria interna
(o fluxo \'e puramente \emph{streaming}); e o caminho cr\'itico deve situar-se no
somador de \num{32} bits que soma os dois produtos da pista, antes do
truncamento.
