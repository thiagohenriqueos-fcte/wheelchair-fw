...
% =============================================================================
\begin{frame}{Síntese: recursos, timing e potência \hfill \scriptsize Vivado 2022.2, xc7z020}
\begin{columns}[T]
...
\vspace{2mm}
\begin{exampleblock}{\footnotesize Timing e potência}
\centering\footnotesize
WNS $=$ \textbf{\SI{+0.872}{\nano\second}} $\Rightarrow$ \alert{fecha a \SI{100}{\mega\hertz}}\\
$F_{\max} = $ \textbf{\SI{109.6}{\mega\hertz}}\\[1mm]
Potência: \textbf{\SI{0.246}{\watt}}\\
{\scriptsize (dinâmica 0,140 W $+$ estática 0,106 W)}
\end{exampleblock}
\centering\tiny 0 erros, 0 \emph{critical warnings}
\end{column}
...
% =============================================================================
\begin{frame}{Block design: ARM + AXI-DMA + acelerador}
\begin{center}
\begin{tikzpicture}[font=\scriptsize, node distance=10mm,
  blk/.style={draw, rounded corners=2pt, minimum height=12mm, minimum width=25mm,
              align=center, inner sep=4pt},
  arr/.style={-Latex, thick}]
  \node[blk, fill=green!12] (ps) {\textbf{Zynq PS}\\ARM Cortex-A9\\\scriptsize DDR: /scan};
  \node[blk, right=20mm of ps, fill=blue!12] (dma) {\textbf{AXI-DMA}};
  \node[blk, right=16mm of dma, fill=orange!18] (acc) {\textbf{lidar\_accel}\\\scriptsize CORDIC + 19 pistas};
  \draw[arr] (ps) -- node[above, font=\tiny] {AXI4-Lite} node[below, font=\tiny] {(GP0)} (dma);
  \draw[arr] (dma) -- node[above, font=\tiny] {AXI4-Stream} (acc);
  \draw[arr] (ps.south) -- ++(0,-6mm) -| node[pos=0.25, below, font=\tiny] {AXI4-Lite: calibração + 19 folgas} (acc.south);
  \draw[arr, red] (acc.north) -- ++(0,6mm) -| node[pos=0.25, above, font=\tiny] {IRQ} (ps.north);
\end{tikzpicture}
\end{center}
...
\item O ARM guarda a varredura na \textbf{DDR}; o \textbf{AXI-DMA} a transmite como
      \emph{stream} ao acelerador (via \texttt{S\_AXI\_HP0})
...
% =============================================================================
% Slide com o print REAL do Vivado. O write_bd_layout gera a página em paisagem
% com /Rotate 270, então a figura entra deitada: o rotatebox{-90} a põe de pé.
% Sem o arquivo, o slide é simplesmente pulado (o TikZ anterior já explica).
\IfFileExists{figs/block_design.pdf}{%
\begin{frame}{Block design no Vivado}
\begin{center}
\resizebox{\textwidth}{!}{\rotatebox{-90}{%
  \includegraphics[trim=211 30 209 36, clip]{figs/block_design.pdf}}}
\end{center}
\centering\tiny
\texttt{zynq} $\to$ \texttt{zynq\_axi\_periph} (AXI4-Lite) $\to$ \texttt{dma} e
\texttt{accel} \quad$\vert$\quad
\texttt{dma/M\_AXIS\_MM2S} $\to$ \texttt{accel/s\_axis} (AXI4-Stream)
\quad$\vert$\quad
\texttt{accel/irq} $\to$ \texttt{IRQ\_F2P}
\end{frame}}{}
