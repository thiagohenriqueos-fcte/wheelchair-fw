...
\emph{Baseline} medido na própria Pi 5. Ciclos de HW \textbf{medidos na simulação}
--- não estimados.
...
% =============================================================================
\begin{frame}{Síntese: recursos, timing e potência \hfill [VIVADO]}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\centering
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Recurso} & \textbf{Usado} & \textbf{\%} \\
\midrule
LUT & \alert{---} & \alert{---} \\
FF & \alert{---} & \alert{---} \\
DSP48 & \alert{---} & \alert{---} \\
BRAM & \alert{---} & \alert{---} \\
\midrule
WNS & \multicolumn{2}{c}{\alert{--- ns}} \\
$F_{\max}$ & \multicolumn{2}{c}{\alert{--- MHz}} \\
Potência & \multicolumn{2}{c}{\alert{--- W}} \\
\bottomrule
\end{tabular}

\vspace{2mm}
\scriptsize \texttt{xc7z020} (ZedBoard)
\end{column}
\begin{column}{0.5\textwidth}
\footnotesize
\textbf{Previsão analítica} (a confrontar):
\begin{itemize}\footnotesize
\item \textbf{78 DSP48} de 220 (\SI{35}{\percent}) \\
      \scriptsize 76 das pistas $+$ 2 da projeção
\item \textbf{0 BRAM} --- não há memória interna, o fluxo é puramente
      \emph{streaming}
\item \textbf{Caminho crítico}: o somador de 32 bits que soma os dois produtos da
      pista, antes do truncamento
\end{itemize}

\vspace{3mm}
\centering
\alert{\scriptsize [COLAR AQUI: print do \\ \texttt{timing\_summary} e \texttt{power}]}
\end{column}
\end{columns}
\end{frame}
